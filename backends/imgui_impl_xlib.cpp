...
// taken from https://github.com/Nemirtingas/imgui/blob/b406428de1af99045bf3f6111ddc10794a40d2d6/backends/imgui_impl_x11.cpp
// repurposed to use ImGui as a window manager
...
struct ImGui_ImplX11_Data
{
    ...
    std::map<Window, ImGui_ImplX11_Frame*>* WindowFrames;
    Window               hFocusWindow;
    Window               hMouseWindow;
    Window               hClipboardWindow;
    ...
    ImGui_ImplX11_Data()      { memset((void*)this, 0, sizeof(*this)); Windows = new std::vector<Window>(); WindowFrames = new std::map<Window, ImGui_ImplX11_Frame*>(); }
};
...
void OnConfigureRequest( XConfigureRequestEvent* pEvent)
{
    XConfigureRequestEvent event = *pEvent;
    ImGui_ImplX11_Data* bd = ImGui_ImplX11_GetBackendData();
    if (bd == NULL)
        return;
    ...
    if (bd->WindowFrames->count(event.window) > 0)
    {
        ImGui_ImplX11_Frame* frame = bd->WindowFrames->at(event.window);
        frame->X = event.x;
        frame->Y = event.y;
        frame->Width = event.width;
        frame->Height = event.height;
    }
    XConfigureWindow(bd->hDisplay, event.window, event.value_mask, &changes);

}

void FrameWindow(Window window, int x, int y, int width, int height)
{
    ImGui_ImplX11_Data* bd = ImGui_ImplX11_GetBackendData();
    if (bd == NULL)
        return;

    if (bd->WindowFrames->count(window) == 0)
    {
        ImGui_ImplX11_Frame* frame = new ImGui_ImplX11_Frame();
        frame->window = XCreateWindow(bd->hDisplay, bd->hWindow, x, y, width, height, 3, CopyFromParent, InputOutput, CopyFromParent, 0, NULL);
        XSelectInput(bd->hDisplay, frame->window, ExposureMask | KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask | StructureNotifyMask);
        XReparentWindow(bd->hDisplay, window, frame->window, 0, 0);
        XMapWindow(bd->hDisplay, frame->window);
        frame->X = x;
        frame->Y = y;
        frame->Width = width;
        frame->Height = height;
        bd->WindowFrames->insert(std::make_pair(window, frame));
    }
}

void OnMapRequest( XMapRequestEvent* pEvent)
{
    XMapRequestEvent event = *pEvent;
    ImGui_ImplX11_Data* bd = ImGui_ImplX11_GetBackendData();
    if (bd == NULL)
        return;

    XWindowAttributes attr;
    XGetWindowAttributes(bd->hDisplay, event.window, &attr);
    FrameWindow(event.window, attr.x, attr.y, attr.width, attr.height);

    XMapWindow(bd->hDisplay, event.window);
}
...
void     ImGui_ImplX11_WindowManager_MakeEachWindow_ImGuiWindow() {
    ImGui_ImplX11_Data* bd = ImGui_ImplX11_GetBackendData();
    if (bd == NULL)
        return;

    ImGui::Begin("Window list");
    ImGui::Text("Window list");
    for (auto it = bd->WindowFrames->begin(); it != bd->WindowFrames->end(); ++it)
    {
        try {
        ImGui_ImplX11_Frame* frame = it->second;
        // get name of window
        XTextProperty text;
        XGetWMName(bd->hDisplay, it->first, &text);
        char* name = (char*)text.value;
        if (name != NULL)
        {
            if (ImGui::Button(name))
            {
                // make the x11 window focused
                XSetInputFocus(bd->hDisplay, it->first, RevertToParent, CurrentTime);

            }
            ImGui::SameLine();
            ImGui::Text("0x%08x", frame->window);
        ...
        XFree(text.value);
            ...
            // window is probably closed, remove it from the list
            delete it->second;
            bd->WindowFrames->erase(it);
            ImGui::End();
            return ImGui_ImplX11_WindowManager_MakeEachWindow_ImGuiWindow();
        }
    ...
    for (auto it = bd->WindowFrames->begin(); it != bd->WindowFrames->end(); ++it)
    {
        ...
        if (name != NULL)
        {
            ImGui::Begin(name);
            inWindow = true;
            // put a blank image the size of the window in the imgui window
            uint32_t* pixels = new uint32_t[frame->Width * frame->Height];
            memset(pixels, 0, frame->Width * frame->Height * sizeof(uint32_t));


            ImGui::Image((void*)(intptr_t)pixels, ImVec2(frame->Width, frame->Height));
            delete[] pixels;

            // make the x11 window appear to be in the imgui window
            XMoveWindow(bd->hDisplay, frame->window, ImGui::GetWindowPos().x + ImGui::GetWindowContentRegionMin().x, ImGui::GetWindowPos().y + ImGui::GetWindowContentRegionMin().y);
            XResizeWindow(bd->hDisplay, it->first, ImGui::GetWindowSize().x, ImGui::GetWindowSize().y);
            XMapWindow(bd->hDisplay, frame->window);

            // if the imgui window is unfocused, unfocus the x11 window
            if (!ImGui::IsWindowFocused())
            {
                XSetInputFocus(bd->hDisplay, PointerRoot, RevertToParent, CurrentTime);
            }
            ...
            // when the imgui window is closed, close the x11 window
            if (!ImGui::IsWindowAppearing())
            {
                // XDestroyWindow(bd->hDisplay, frame->window);
                // bd->WindowFrames->erase(it);
                // delete frame;
            }
            // when the x11 window is closed, close the imgui window
            if (XGetWindowAttributes(bd->hDisplay, frame->window, &frame->attr) == 0)
            {
                // ImGui::End();
                // bd->WindowFrames->erase(it);
                // delete frame;
